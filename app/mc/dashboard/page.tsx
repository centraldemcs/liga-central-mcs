...
export default function MCDashboard() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (mcData.foto_url) setFotoUrl(mcData.foto_url)
  ...
  async function handleFotoUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const { data: urlData } = supabase.storage.from('fotos-mcs').getPublicUrl(path)
    const url = urlData.publicUrl

    await supabase.from('mcs').update({ foto_url: url }).eq('id', mc.id)
    setFotoUrl(url)
    setUploadando(false)
  }
  ...
  confrontos.forEach(c => {
    ...
    const adversarioNome = c.mc_a_id === mc.id ? (c.mcs_b as any)?.nome_artistico : (c.mcs_a as any)?.nome_artistico
    if (!adversarioNome) return
    if (!adversarioMap[adversarioId]) adversarioMap[adversarioId] = { nome: adversarioNome, total: 0, vitorias: 0, derrotas: 0 }
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          <div style={{ position: 'relative' }}>
            <div
              ...
              style={{ width: '72px', height: '72px', borderRadius: '50%', background: fotoUrl ? 'transparent' : 'rgba(0,0,0,0.2)', border: '3px solid #111', overflow: 'hidden', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}
            >
              {fotoUrl ? (
                <img src={fotoUrl} alt={mc.nome_artistico} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              ) : (
                <span style={{ fontFamily: 'Barlow Condensed, sans-serif', fontWeight: 900, fontSize: '24px', color: '#111' }}>
                  {mc.nome_artistico.charAt(0).toUpperCase()}
                </span>
              )}
            </div>
            <div
      ...
      <div style={{ padding: '40px', maxWidth: '900px', margin: '0 auto' }}>
        ...
        {adversarios.length > 0 && (
          <div style={{ marginBottom: '32px' }}>
            <h2 style={{ fontFamily: 'Barlow Condensed, sans-serif', fontWeight: 900, fontStyle: 'italic', fontSize: '28px', textTransform: 'uppercase', color: '#fff', marginBottom: '16px', borderBottom: '1px solid rgba(245,168,0,0.2)', paddingBottom: '12px' }}>Rivalidades</h2>
        ...
        <div>
          <h2 style={{ fontFamily: 'Barlow Condensed, sans-serif', fontWeight: 900, fontStyle: 'italic', fontSize: '28px', textTransform: 'uppercase', color: '#fff', marginBottom: '16px', borderBottom: '1px solid rgba(245,168,0,0.2)', paddingBottom: '12px' }}>Histórico</h2>
          {historico.length === 0 ? (
