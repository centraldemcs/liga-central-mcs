...
export default function DashboardBatalha() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: batalhaData } = await supabase.from('batalhas').select('*').eq('profile_id', user.id).single()
      if (!batalhaData) { router.push('/batalha/login'); return }
      setBatalha(batalhaData)
      if (batalhaData.logo_url) setLogoUrl(batalhaData.logo_url)
      const { data: noitesData } = await supabase.from('noites').select('*, confrontos(count)').eq('batalha_id', batalhaData.id).order('data', { ascending: false }).limit(5)
      setNoites(noitesData || [])
      const { data: ptsData } = await supabase.from('pontuacoes').select('mc_id, pontos, mcs(nome_artistico, cidade)').eq('batalha_id', batalhaData.id)
      if (ptsData) {
        const map: any = {}
  ...
  async function handleLogoUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file || !batalha) return
    setUploadando(true)
    const ext = file.name.split('.').pop()
    const path = batalha.id + '.' + ext
    const { error: uploadError } = await supabase.storage.from('logos-batalhas').upload(path, file, { upsert: true })
    if (uploadError) { setUploadando(false); return }
    const { data: urlData } = supabase.storage.from('logos-batalhas').getPublicUrl(path)
    const url = urlData.publicUrl
    await supabase.from('batalhas').update({ logo_url: url }).eq('id', batalha.id)
    setLogoUrl(url)
    setUploadando(false)
  }
    ...
    <main style={{ background: '#111', minHeight: '100vh', fontFamily: 'Barlow, sans-serif' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          <div style={{ position: 'relative' }}>
            <div
              ...
              style={{ width: '72px', height: '72px', borderRadius: '8px', background: logoUrl ? 'transparent' : 'rgba(0,0,0,0.2)', border: '3px solid #111', overflow: 'hidden', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}
            >
              {logoUrl ? (
                <img src={logoUrl} alt={batalha.nome} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              ) : (
                <span style={{ fontFamily: 'Barlow Condensed, sans-serif', fontWeight: 900, fontSize: '24px', color: '#111' }}>
                  {batalha.nome.charAt(0).toUpperCase()}
                </span>
              )}
            </div>
            <div
              onClick={() => fileInputRef.current?.click()}
              style={{ position: 'absolute', bottom: 0, right: 0, width: '22px', height: '22px', background: '#111', borderRadius: '50%', border: '2px solid #F5A800', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: '11px' }}
            >
              {uploadando ? '⏳' : '📷'}
            </div>
      ...
      <div style={{ padding: '40px', maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '32px' }}>
          <h2 style={{ fontFamily: 'Barlow Condensed, sans-serif', fontWeight: 900, fontStyle: 'italic', fontSize: '28px', textTransform: 'uppercase', color: '#fff', marginBottom: '16px', borderBottom: '1px solid rgba(245,168,0,0.2)', paddingBottom: '12px' }}>MCs da Batalha</h2>
          {mcs.length === 0 ? (
        ...
        <div>
          <h2 style={{ fontFamily: 'Barlow Condensed, sans-serif', fontWeight: 900, fontStyle: 'italic', fontSize: '28px', textTransform: 'uppercase', color: '#fff', marginBottom: '16px', borderBottom: '1px solid rgba(245,168,0,0.2)', paddingBottom: '12px' }}>Últimas Noites</h2>
          {noites.length === 0 ? (
            <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', padding: '32px', textAlign: 'center' }}>
